...
function encryptText(plainText) {
	...
	const encryptedText = Buffer.concat([cipher.update(String(plainText ?? ''), 'utf8'), cipher.final()]);
...
export function validateDataEncryptionConfig() {
	getKeyFromEnv();
}
...
export function encryptJson(value) {
	...
	if (serializedValue === undefined) {
		throw new Error('El valor a cifrar es requerido.');
	}

	return encryptText(serializedValue);
}
