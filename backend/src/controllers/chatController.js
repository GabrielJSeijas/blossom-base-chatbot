...
function buildConversationSummaryFromMessages(messages) {
  ...
  if (latestAssistantSnippets.length > 0) {
    ...
    latestAssistantSnippets.forEach(snippet => summaryLines.push(`- ${snippet}`));
  }
...
function countStableMessagesForDecay(previousRiskAssessments, anchorScore) {
  if (!Array.isArray(previousRiskAssessments) || previousRiskAssessments.length < 2) return 0;
  let stableCount = 0;
...
export async function chatController(req, res) {
  try {
    ...
    try {
      ...
      if (riskAssessment.should_alert && ['high', 'critical'].includes(riskAssessment.risk_level)) {
        ...
        (async () => {
          try {
            ...
            console.error('❌ Error enviando alerta de Brevo en background:', err?.message || err);
          }
        })();
      }
...
export async function getChatHistoryController(req, res) {
  try {
    ...
    const latestConversationId = messages.length > 0 ? messages[messages.length - 1].conversationId : null;
