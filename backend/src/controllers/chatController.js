...
const SUMMARY_REBUILD_EVERY_MESSAGES = Number(
  process.env.CONVERSATION_SUMMARY_REBUILD_EVERY || 8,
);
const RISK_DECAY_HOLD_MESSAGES = Number(
  process.env.RISK_DECAY_HOLD_MESSAGES || 5,
);
const RISK_DECAY_STEP_MESSAGES = Number(
  process.env.RISK_DECAY_STEP_MESSAGES || 5,
);
const RISK_DECAY_LOOKBACK_MESSAGES = Number(
  process.env.RISK_DECAY_LOOKBACK_MESSAGES || 40,
);
...
function normalizeRiskAssessmentShape(riskAssessment) {
  return {
    ...
    confidence: Number.isFinite(Number(riskAssessment?.confidence))
      ? Number(riskAssessment.confidence)
      : 0,
    summary_for_moderator: String(
      riskAssessment?.summary_for_moderator || "",
    ).trim(),
    recommended_bot_mode: String(
      riskAssessment?.recommended_bot_mode || "normal",
    )
      .trim()
      .toLowerCase(),
  };
...
function buildConversationSummaryFromMessages(messages) {
  ...
  for (const messageDocument of messages) {
    const content = normalizeTextSnippet(
      decryptMessage(messageDocument.content),
    );
    if (!content) continue;
  ...
  if (userSnippets.length === 0 && assistantSnippets.length === 0) return "";
  const summaryLines = [];
  ...
  if (latestUserSnippets.length > 0) {
    ...
    latestUserSnippets.forEach((snippet) => summaryLines.push(`- ${snippet}`));
  }
  if (latestAssistantSnippets.length > 0) {
    summaryLines.push("Apoyo previo del asistente:");
    latestAssistantSnippets.forEach((snippet) =>
...
function formatConversationSummaryDocument(document) {
  if (!document) return { summaryText: "", summaryMessageCount: 0 };
  const summaryData = document.encryptedData
    ? decryptJson(document.encryptedData)
    : document;
  return {
...
async function saveConversationSummary({
  ...
  await conversationsCollection.updateOne(
    { _id: conversationId, userId: userObjectId },
    {
      $set: {
        userId: userObjectId,
        encryptedData: encryptJson({ summaryText, summaryMessageCount }),
        updatedAt: now,
      },
...
function countStableMessagesForDecay(previousRiskAssessments, anchorScore) {
  if (
    !Array.isArray(previousRiskAssessments) ||
    previousRiskAssessments.length < 2
  )
    return 0;
  let stableCount = 0;
...
function applyRiskCooldown(riskAssessment, previousRiskAssessments) {
  ...
  const previousRiskAssessment = Array.isArray(previousRiskAssessments)
    ? previousRiskAssessments[0]
    : null;

  if (!previousRiskAssessment) return current;

  const previousLevel = String(
    previousRiskAssessment.riskLevel || "none",
  ).toLowerCase();
  const previousLevelScore = getRiskLevelScore(previousLevel);
  const incomingLevelScore = getRiskLevelScore(current.risk_level);

  if (
    incomingLevelScore >= previousLevelScore ||
    previousLevelScore < getRiskLevelScore("high")
  ) {
    return current;
  }

  const stableMessagesBeforeCurrent = countStableMessagesForDecay(
  ...
  if (stableMessagesIncludingCurrent > RISK_DECAY_HOLD_MESSAGES) {
    const messagesAfterHold =
      stableMessagesIncludingCurrent - RISK_DECAY_HOLD_MESSAGES;
    maxDrop =
      1 +
      Math.floor(
        (messagesAfterHold - 1) / Math.max(RISK_DECAY_STEP_MESSAGES, 1),
      );
  }

  const allowedScore = Math.max(
    previousLevelScore - maxDrop,
    incomingLevelScore,
  );
  if (allowedScore === incomingLevelScore) return current;

  const adjustedLevel = scoreToRiskLevel(allowedScore);
  const adjustedUrgency = maxUrgency(
    current.urgency,
    getMinimumUrgencyForRiskLevel(adjustedLevel),
  );
  const mergedCategories = Array.from(
    new Set([
      ...(previousRiskAssessment.categories || []),
      ...(current.categories || []),
    ]),
  ).slice(0, 12);
  ...
  return {
    ...
    should_alert:
      getRiskLevelScore(adjustedLevel) >= getRiskLevelScore("medium"),
...
function formatRiskAssessmentDocument(document) {
  ...
  const riskData = document.encryptedData
    ? decryptJson(document.encryptedData)
    : document;
  return {
...
function formatUserLabel(userDocument, fallbackId) {
  ...
  return (
    ...
    userDocument.email ||
    `Usuario ${String(fallbackId).slice(-6)}`
  );
}

function shouldCreateRiskAlert(riskAssessment) {
  return (
    ["high", "critical"].includes(riskAssessment?.risk_level) ||
    Boolean(riskAssessment?.should_alert)
  );
}
...
export async function chatController(req, res) {
  try {
    ...
    const storedConversationMessages = await messagesCollection
      ...
      .sort({ createdAt: 1, _id: 1 })
      .toArray();
    const olderMessages = storedConversationMessages.slice(
      0,
      -MAX_LLM_CONTEXT_MESSAGES,
    );
    let { summaryText, summaryMessageCount } =
      await getConversationSummaryState(conversationId, userObjectId);

    if (olderMessages.length > 0) {
      const shouldRebuildSummary =
        !summaryText ||
        storedConversationMessages.length - summaryMessageCount >=
          SUMMARY_REBUILD_EVERY_MESSAGES;
    ...
    const conversationHistory = storedConversationMessages
    ...
    const previousRiskDocuments =
      await getRecentRiskAssessmentsForUser(userObjectId);
    ...
    try {
      riskAssessment = await classifyRisk(
        normalizedMessage,
        conversationHistory,
      );
      ...
      // 2. DISPARAR ALERTA DE CORREO ELECTRÓNICO (Asíncrona y con resolución de nombre)
      if (
        riskAssessment.should_alert &&
        ["high", "critical"].includes(riskAssessment.risk_level)
      ) {
        // Ejecutamos en segundo plano para proteger el tiempo de respuesta de la API hacia el usuario
        (async () => {
          try {
            ...
            const resolvedLabel = formatUserLabel(userDoc, userObjectId);
            ...
            console.error(
              "❌ Error enviando alerta de Brevo en background:",
              err?.message || err,
            );
          }
        })();
      ...
      console.error(
        "[RISK] No se pudo clasificar o guardar el riesgo:",
        error?.message || error,
      );
      riskAssessment = applyRiskCooldown(
        {
    ...
    const formattedRiskAssessment =
      formatRiskAssessmentDocument(savedRiskAssessment);
    ...
    try {
      ...
      console.error(
        "[CHAT] Error generando respuesta:",
        error?.message || error,
      );
    ...
    console.error(
      "[CHAT] Error inesperado en chatController:",
      error?.message || error,
    );
    res
      .status(500)
      .json({ error: error.message || "Error procesando el chat." });
  }
}
...
export async function getChatHistoryController(req, res) {
  try {
    ...
    res
      .status(500)
      .json({ error: error.message || "No se pudo cargar el historial." });
  }
}
...
export async function deleteChatHistoryController(req, res) {
  try {
    ...
    // 1. Validar consentimiento explícito (Criterio de seguridad GDPR)
    const { confirmation } = req.body ?? {};
    if (confirmation !== true) {
      res.status(400).json({
        error:
          "Se requiere confirmación explícita (confirmation: true) para eliminar el historial.",
      });
      return;
    }
    ...
    // 3. Eliminar resúmenes de conversaciones (metadatos de contexto)
    const conversationsResult = await conversationsCollection.deleteMany({
      userId: userObjectId,
    });

    // NOTA CRÍTICA: NO eliminamos risk_assessments ni risk_alerts.
    // Esto cumple con HIPAA/GDPR para auditoría de seguridad y alertas de riesgo.
    ...
    res.json({
      ...
      message:
        "Historial de conversaciones eliminado de forma segura e irreversible.",
      timestamp: new Date(),
    });
  } catch (error) {
    console.error(
      "[CHAT] Error eliminando historial:",
      error?.message || error,
    );
    res.status(500).json({ error: "No se pudo eliminar el historial." });
  }
}
...
export async function getLatestRiskAssessmentController(req, res) {
  try {
    ...
    const groupedRisks = await riskAssessmentsCollection
      .aggregate(pipeline)
      .toArray();
    ...
    const alerts = groupedRisks.map((item) => {
      ...
      return {
        ...
        categories: Array.isArray(formattedRisk?.categories)
          ? formattedRisk.categories
          : [],
    ...
    console.error(
      "[RISK] Error cargando el último riesgo:",
      error?.message || error,
    );
    res
      .status(500)
      .json({ error: error.message || "No se pudo cargar el riesgo." });
  }
}
